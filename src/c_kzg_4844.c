...
C_KZG_RET verify_kzg_proof(bool *out,
    ...
    if (ret != C_KZG_OK) return ret;
    ret = bytes_to_g1(&c, commitment);
    if (ret != C_KZG_OK) return ret;
    ret = bytes_to_g1(&p, kzg_proof);
    if (ret != C_KZG_OK) return ret;
...
C_KZG_RET verify_aggregate_kzg_proof(bool *out,
    ...
    if (ret != C_KZG_OK) goto out;
    ret = bytes_to_g1(&p, kzg_aggregated_proof);
    if (ret != C_KZG_OK) goto out;

    ret = verify_kzg_proof_impl(out, &aggregated_poly_commitment, &evaluation_challenge, &y, &p, s);

out:
    if (0 < n && polys != NULL) free(polys);
    if (0 < n && commitments != NULL) free(commitments);
    return ret;
}
