...
func GetFieldElementsPerBlob() int {
	if !loaded {
		panic("trusted setup isn't loaded")
	}
	return int(settings.field_elements_per_blob)
}

func GetBytesPerBlob() int {
	if !loaded {
		panic("trusted setup isn't loaded")
	}
	return int(settings.bytes_per_blob)
}
